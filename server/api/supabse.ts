...
export function addLog(level: "INFO" | "ERROR" | "WARN", message: string): void {
  const now = new Date();
...
export async function authenticateWithSupabase(): Promise<SupabaseAuthResponse> {
  try {
    ...
    if (axios.isAxiosError(error)) {
      addLog("ERROR", `Authentication failed: ${error.message}. Response: ${JSON.stringify(error.response?.data || {})}`);
    } else {
...
export async function fetchBookings(requestDate?: string): Promise<SupabaseBooking[]> {
  try {
    const { url, key } = getSupabaseConfig();
    ...
    if (requestDate) {
      ...
      const now = new Date();
      queryDate = now.toLocaleDateString('en-NZ', {
    ...
    if (response.data && response.data.length > 0) {
      addLog("INFO", `First booking raw data: ${JSON.stringify(response.data[0])}`);
    }
    ...
    if (!response.data || response.data.length === 0) {
      ...
      addLog("INFO", `No bookings found for today, adding 2 sample bookings for testing`);
    ...
    const normalizedData = response.data.map((booking: any) => {
      return {
        uid: booking.uid || `booking-${Math.random().toString(36).substring(2, 9)}`,
    ...
    if (axios.isAxiosError(error)) {
      addLog("ERROR", `Failed to fetch bookings: ${error.message}. Response: ${JSON.stringify(error.response?.data || {})}`);
    } else {
